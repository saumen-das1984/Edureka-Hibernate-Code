package com.hibernate.filter.anno.app;
...
import com.hibernate.filter.anno.model.Branch;
import com.hibernate.filter.anno.util.HibernateUtil;
...
public class DataHandler {
	...
	private void populateBranches() {
		...
		try {
			...
			session.save(new Branch("TestBranch3", true));
			session.save(new Branch("TestBranch4", true));
			session.save(new Branch("BranchSub3", true));
			session.save(new Branch("BranchSub4", true));
	...
	private void testFilter() {
		...
		try {
			...
			filter.setParameter("branchNameFilterParam", "TestBranch4");
