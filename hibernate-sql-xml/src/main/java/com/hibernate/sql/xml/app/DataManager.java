package com.hibernate.sql.xml.app;
...
import com.hibernate.sql.xml.model.Employee;
import com.hibernate.sql.xml.util.HibernateUtil;
...
public class DataManager {
	...
	private static void sqlNamedQuery() {
		...
		try {
			...
			Query namedSqlQuery = session.getNamedSQLQuery("namedEmployeeQuery");
			namedSqlQuery.setInteger("empId", 1003);
			
			@SuppressWarnings("unchecked")
			List<Object> list = namedSqlQuery.list();
			System.out.println("Size of Employee List : "+list.size());
			for (Object obj : list) {
				System.out.println(obj);
			}
